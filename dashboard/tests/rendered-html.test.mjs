...
test("server-renders the Autoprover campaign control room", async () => {
  ...
  assert.match(html, /Problem queue/);
...
test("keeps campaign controls, completed reports, and metadata wired", async () => {
  ...
  assert.match(component, /Expected value \+ verifier leverage/);
  ...
  assert.match(component, /Find more/);
  assert.match(component, /Run next/);
  assert.match(component, /Switch out/);
  assert.match(component, /Recorded attempts/);
